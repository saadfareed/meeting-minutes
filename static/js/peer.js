...
btnRecordScreen = document.querySelector('#btn-record-screen');
...
var div_creator = document.querySelector("#chat");
...
btnJoin.onclick = () => {
    ...
    if(regex.test(roomname)){
        console.log("Correct roomname");
    }
    else{
        document.querySelector('#label-error').innerHTML = "Room Name must have Alphnumeric, undersocre, hiphen and dot";
        return;
    }
    if(username == '' & roomname == ''){
        // ignore if username and roomname is empty
        return;
    }
    ...
    document.querySelector('#label-roomname').innerHTML = "Room Name : "+roomname;
    document.querySelector('#label-username').innerHTML = "User Name : "+username;
    

    webSocket = new WebSocket(endPoint);

    webSocket.onopen = function(e){
        console.log('Connection opened! 111- ', e);
        console.log('line 112 roomname',roomname);
        // notify other peers
        sendSignal(roomname,'new-peer', {
    ...
    webSocket.onclose = function(e){
        ...
        // breakPoint = clearInterval(breakPoint);
        console.log("clearInterval is done ! @@@@@@@@@@@@@@@@@@@@@@")
        // SaveVedioServerSide(wavfromblob);
        SaveVedio(wavfromblob);
    }
...
function webSocketOnMessage(event){
    ...
    if (parsedData['message']['error']){
        ...
        document.querySelector('#label-roomname').innerHTML = "Room Name Not Found, Please try again! <br> Room Name :";
        document.querySelector('#label-username').innerHTML = "User Name : ";
    ...
    console.log("-------parsedData['message']['isCreator']",parsedData['message']['isCreator'])
    if(parsedData['message']['isCreator'] && username==parsedData['peer']){
        if(!btnRecordScreen){
            var btn_creator = document.createElement("button");
            btn_creator.id = 'btn-record-screen'
            btn_creator.innerHTML = 'Double Click to Start Record'
            ...
            .then(stream => {
                ...
                btnRecordScreen.addEventListener('click', () => {
                    console.log("recording at line 198 ",recording);
                    // setRecordingStart();
                    if(recording){
                        // toggle recording
                        recording = !recording;
                        // setRecordingStart();
                        console.log("recording at line 204 ",recording);
                        btnRecordScreen.innerHTML = 'Record Screen';

                        recorder.stopRecording(function() {
                            var blob = recorder.getBlob();
                            console.log("209 line++++++++++++++video recording off!++++++++++++++++++++++++++++++++")
                            var wavfromblob = new File([blob], "incomingaudioclip.wav");
                            SaveVedio(wavfromblob);
                        });
            ...
            .then(e => {
                    ...
                    //         // breakPoint = setInterval( PassServerSide, 9000);
                    //         // recorder.save();
...
function SaveVedioServerSide(blob){
    try{
        var data = new FormData();
        data.append('video', blob);
        // data.append('video', );  //Correct: sending the Blob itself
        data.append('roomname', roomname);  //Correct: sending the Blob itself
        data.append('username',username)
        
        console.log("data at line 299",data);
        $.ajax({
                type: "POST",
...
window.addEventListener("beforeunload", function (e) {
    // if(recording){
        // toggle recording
        // recording = !recording;
        // console.log("recording at line 421 ",recording);
        // btnRecordScreen.innerHTML = 'Record Screen';
        ...
        recorder.stopRecording(function() {
            ...
            var wavfromblob = new File([blob], "incomingaudioclip.wav");
            var data = new FormData();
            data.append('video', wavfromblob);  //Correct: sending the Blob itself
            console.log("data at line 327",data);
            data.append('roomname', roomname);  //Correct: sending the Blob itself
            data.append('username',username)
            $.ajax({
                type: "POST",
...
function checkRecordingOn(){
    ...
    $.ajax({
        ...
        timeout: 600000,
        success: function (data) {
            console.log("data at line 436",data)
            // if(data['success']){
                console.log("line 438 recording starting");
                // toggle recording
                recording = !recording;
                ...
                //     .then(stream => {
                    ...
                    // });
                    // recorder.startRecording();
                    console.log("line 489 recording started");
                // }
        },
        error: function (e) {
            console.log("ERROR : ", e);
...
function setRecordingStart(){
    var data = new FormData();
    data.append('recording', recording);  //Correct: sending the Blob itself
    data.append('roomname', roomname);  //Correct: sending the Blob itself
    data.append('username',username)
    $.ajax({
        type: "POST",
...
userMedia = navigator.mediaDevices.getUserMedia(constraints)
    .then(stream => {
        ...
        console.log("recording at line 422 ",recording);
            
            if(recording){
                // toggle recording
                recording = !recording;
                console.log("recording at line 426 ",recording);
                btnRecordScreen.innerHTML = 'Record Screen';
...
function sendSignal(roomname_,action, message){
    console.log("++++++611+++++message",message)
    webSocket.send(
        JSON.stringify(
            {
                'roomname':roomname_,
...
function PassServerSide(){

    recorder.stopRecording(function() {
        var blob = recorder.getBlob();
        console.log("++++++++++++++video recording off 536!++++++++++++++++++++++++++++++++")
        var wavfromblob = new File([blob], "incomingaudioclip.wav");
        // breakPoint = clearInterval(breakPoint);
        // console.log("clearInterval is done ! @@@@@@@@@@@@@@@@@@@@@@")
        SaveVedioServerSide(wavfromblob);
...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                ...
                console.log("peerUsername",peerUsername,"peer",peer)
                // checkRecordingOn();
                delete mapPeers[peerUsername];
                if(iceConnectionState != 'closed'){
                    console.log("line 671 connection closed!")
                    peer.close();
                }
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                ...
                if(iceConnectionState != 'closed'){
                    console.log("line 695 connection closed!")
                    peer.close();
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                ...
                if(iceConnectionState != 'closed'){
                    console.log("line 714 connection closed!")
                    peer.close();
                }
            }
        };
    ...
    peer.createOffer()
        .then(o => {
            ...
            console.log('Setting local offer for %s.', peerUsername,"o",o);
...
function createAnswerer(offer, peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        peer.ondatachannel = e => {
            ...
            console.log("at line 802 peer, peer.dc",peer, peer.dc,peerUsername)
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                ...
                if(iceConnectionState != 'closed'){
                    ...
                    console.log("line 791 connection closed!")
                    peer.close();
                }
                removeVideo(remoteVideo);
            }
        ...
        peer.ondatachannel = e => {
            ...
            peer.oniceconnectionstatechange = () => {
                ...
                if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                    delete mapScreenPeers[peerUsername];
                    if(iceConnectionState != 'closed'){
                        console.log("line 823 connection closed!")
                        peer.close();
                    }
                }
            };
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed"){
                delete mapPeers[peerUsername + ' Screen'];
                if(iceConnectionState != 'closed'){
                    console.log("line 859 connection closed!")
                    peer.close();
                }
                removeVideo(remoteVideo);
            }
    ...
    peer.onicecandidate = (event) => {
        ...
        sendSignal(roomname,'new-answer', {
            'sdp': peer.localDescription,
            'receiver_channel_name': receiver_channel_name,
            'local_screen_sharing': localScreenSharing,
            'remote_screen_sharing': remoteScreenSharing,
    ...
    peer.setRemoteDescription(offer)
        ...
        .then(a => {
            ...
            console.log('Setting local answer for %s.', peerUsername,"a",a);
            peer.setLocalDescription(a);
        })
        .then(() => {
            console.log('Answer created for %s.', peerUsername);
            // a = peer.localDescription['recording'] = recording;
            // print("a at line 931----",a)
            // peer.setLocalDescription(a);
